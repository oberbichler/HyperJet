...
auto bind(py::module &m, const std::string &name)
{
    ...
    cls
        .def_property_readonly_static("is_dynamic", [](py::object)
                                      { return T::is_dynamic(); })
        .def_property_readonly_static("order", [](py::object)
                                      { return T::order(); });
    ...
    if constexpr (T::is_dynamic())
    {
        cls
            .def_static(
                "variables", [](const std::vector<typename T::Scalar> &values)
    ...
    cls
        ...
        .def("__copy__", [](const T &self)
             { return self; })
        .def(
            "__deepcopy__", [](const T &self, py::dict &memo)
            { return self; },
            "memodict"_a);
