...
template <typename T> auto bind(py::module &m, const std::string &name) {
  ...
  // constructor
  cls.def(py::init(
              py::overload_cast<typename T::Scalar, hj::index>(&T::constant)),
          "f"_a = 0, "size"_a)
      .def(py::init(py::overload_cast<const typename T::Data &>(&T::create)),
           "data"_a)
      .def(py::init(py::overload_cast<typename T::Scalar>(&T::constant)),
           "f"_a = 0);

  if constexpr (T::order() == 1) {
    cls.def(py::init(&T::from_gradient), "f"_a, "g"_a);
  } else {
    cls.def(py::init(&T::from_arrays), "f"_a, "g"_a, "hm"_a);
  }
  ...
  cls.def_property_readonly_static("is_dynamic",
                                   [](py::object) { return T::is_dynamic(); })
      .def_property_readonly_static("order",
                                    [](py::object) { return T::order(); });

  // read-only properties
  cls.def_property_readonly("data", py::overload_cast<>(&T::adata))
      .def_property_readonly("g", py::overload_cast<>(&T::ag))
      .def_property_readonly("size", &T::size);

  // static methods
  cls.def_static("constant",
                 py::overload_cast<typename T::Scalar, hj::index>(&T::constant),
                 "f"_a, "size"_a)
      .def_static("constant",
                  py::overload_cast<typename T::Scalar>(&T::constant), "f"_a)
      .def_static("empty", py::overload_cast<>(&T::empty))
      .def_static("empty", py::overload_cast<hj::index>(&T::empty), "size"_a)
      .def_static("variable",
                  py::overload_cast<hj::index, typename T::Scalar, hj::index>(
                      &T::variable),
                  "i"_a, "f"_a, "size"_a)
  ...
  if constexpr (T::is_dynamic()) {
    cls.def_static(
        "variables",
        [](const std::vector<typename T::Scalar> &values) {
    ...
    cls.def_static(
           "variable",
           py::overload_cast<hj::index, typename T::Scalar>(&T::variable),
           "i"_a, "f"_a)
        .def_static(
            "variables",
  ...
  cls.def("reciprocal", &T::reciprocal)
      ...
      .def("hypot",
           py::overload_cast<const T &, const T &, const T &>(&T::hypot));
  ...
  cls.def("exp", &T::exp)
      ...
      .def("log", py::overload_cast<typename T::Scalar>(&T::log, py::const_),
           "base"_a)
      .def("log2", &T::log2)
      .def("log10", &T::log10);
  ...
  // serialization
  cls.def(py::pickle([](const T &self) { return py::make_tuple(self.m_data); },
                     [](py::tuple tuple) {
                       ...
                       if constexpr (T::is_dynamic()) {
                         return T(data,
                                  T::size_from_data_length(hj::length(data)));
                       } else {
                         return T(data);
                       }
                     }))
      ...
      .def(
          "__deepcopy__", [](const T &self, py::dict &memo) { return self; },
          "memodict"_a);
