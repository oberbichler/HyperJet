...
BENCHMARK_TEMPLATE(method, 8)->MinTime(2.0); \
...
static void mul_autodiff(benchmark::State& state) {
    Vector8dual2nd x;
    x << 1.0, 2.0, 3.0, 4.0, 5, 6, 7, 8;

    for (auto _ : state) {
        auto d = hessian(m, wrt(x), at(x));

        benchmark::DoNotOptimize(d);
    }
}
BENCHMARK(mul_autodiff)->MinTime(2.0);
...
dual2nd f(Vector8dual2nd x)
{
    using std::sqrt;
    return sqrt(x[3] * (x[3] / x[2]));
}

static void div_autodiff(benchmark::State& state) {
    Vector8dual2nd x;
    x << 1.0, 2.0, 3.0, 4.0, 5, 6, 7, 8;

    for (auto _ : state) {
        auto d = hessian(f, wrt(x), at(x));

        benchmark::DoNotOptimize(d);
    }
}
BENCHMARK(div_autodiff)->MinTime(2.0);
...
static void sqrt_autodiff(benchmark::State& state) {
    Vector8dual2nd x;
    x << 1.0, 2.0, 3.0, 4.0, 5, 6, 7, 8;
    ...
    for (auto _ : state) {
        auto d = hessian(g, wrt(x), at(x));

        benchmark::DoNotOptimize(d);
    }
}
BENCHMARK(sqrt_autodiff)->MinTime(2.0);
